...
class DbPostgresql:
		...
		if config.DATABASE is not None:
			if self.dbname in config.DATABASE:
				try:
					...
					raise Exception('Error during connection to postgres DB')
	...
	def execute(self, query, args=None, commit=True):
		...
		:param args: dict to replace %s in query
		...
		if self.cnx is not None:
			...
			try:
				...
				raise Exception('Error is not on query : %s, query : %s' % (e, query))
			finally:
				cursor.close()
		else:
	...
	def get_all(self, query, args=None, params=None):
		...
		:param query: query to run
		:param args: dict to replace %s in query
		:param params: dict woth parameter for SQL function
